...
```{r}
subset(mpg, class == "subcompact" & cty < 16, select = c('manufacturer', 'model'))
...
```{r}
 ggplot(data = mpg) + 
      geom_point(mapping = aes(x = displ, y = hwy, shape = class)) + scale_shape_manual(values = c("2seater" = 16, "compact" = 17, "midsize" = 15, "minivan" = 14, "pickup" = 13, "subcompact" = 12, "suv" = 11))
```
...
sizeMass <- cbind(sizeMass, mass_log10, size_log10)
